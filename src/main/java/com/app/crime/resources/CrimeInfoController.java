...
public class CrimeInfoController {
	...
	public List<Object> getCategories() {
		String urlCrimeCategories = "https://data.police.uk/api/crime-categories";
	...
	public Object getCrimesAtLocation(@PathVariable("inputPostcode") String inputPostcode,
		...
		String urlCrimeLocation = "https://data.police.uk/api/crimes-at-location?date=" + inputDate + "&lat=" + latitude
				+ "&lng=" + longitude;

		Object locationCrimes = restTemplate.getForObject(urlCrimeLocation, Object.class);

		if (locationCrimes.toString().length() == 2) { // where the response returns [] empty, .isEmpty() & .isBlank()
														// not working
			throw new DataNotFoundException("No data found");
		}
	...
	public boolean isPostcodeValid(String inputPostcode) {
		Postcode postcodeObject = restTemplate
				.getForObject("https://api.postcodes.io/postcodes/" + inputPostcode + "/validate", Postcode.class);
